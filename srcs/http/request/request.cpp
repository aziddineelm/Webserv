...
std::string Request::getHeader(const std::string& key) const {
	std::map<std::string, std::string>::const_iterator it = _headers.find(_toLower(key));
	if (it != _headers.end())
		return it->second;
	return "";
}
...
// Returns false if the string is empty, contains non-numeric chars, or is negative.
